...
parser.add_argument('--distractor1_path', type=str, help='Load path of distractor1')
parser.add_argument('--distractor2_path', type=str, help='Load path of distractor2')
...
def main(args):
    ...
    with open(args.distractor1_path, 'r') as f:
        all_distractor1 = [a.rstrip() for a in f.readlines()]

    with open(args.distractor2_path, 'r') as f:
        all_distractor2 = [a.rstrip() for a in f.readlines()]

    all_generated = []

    for context, question_answer, distractor1, distractor2 in zip(all_context, all_question_answer, all_distractor1, all_distractor2):
        combo = context + " [SEP] " + question_answer + " [SEP] " + distractor1 + " [SEP] " + distractor2
